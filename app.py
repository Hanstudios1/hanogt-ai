...
import os
import json
from PIL import Image, ImageDraw, ImageFont # Görsel işleme için
import time
from io import BytesIO # Avatar ve görsel indirme için
from duckduckgo_search import DDGS # Web araması için
from urllib.parse import urlparse # URL işleme için
import google.generativeai as genai # Gemini AI için

# --- Sabitler ---
CHAT_HISTORY_FILE = "chat_history.json"
DEFAULT_ERROR_MESSAGE = "Üzgünüm, bir şeyler ters gitti. Lütfen tekrar deneyin."
REQUEST_TIMEOUT = 10 # Web istekleri için sn cinsinden zaman aşımı
SCRAPE_MAX_CHARS = 1000 # Web kazıma için maks karakter
GEMINI_ERROR_PREFIX = "GeminiError:" # Gemini hatalarını ayırt etmek için

# --- Bilgi Tabanı (Varsa Yükle) ---
try:
    ...
    # Eğer dosya yoksa, boş fonksiyonlar tanımla
...
st.set_page_config(
    ...
    page_icon="🤖",  # Emoji ikon
    layout="wide",  # Geniş mod
    initial_sidebar_state="collapsed" # Sidebar başlangıçta kapalı (artık kullanmıyoruz ama kalsa da olur)
)

# --- API Anahtarı ve Gemini Yapılandırması ---
# Streamlit Secrets'tan güvenli bir şekilde API anahtarını oku
api_key = st.secrets.get("GOOGLE_API_KEY") # .get() kullanmak daha güvenli, yoksa None döner
gemini_model = None
gemini_init_error = None

if not api_key:
    gemini_init_error = "🛑 Google API Anahtarı Secrets'ta bulunamadı! Lütfen `.streamlit/secrets.toml` dosyasını veya Streamlit Cloud Secrets ayarlarını yapılandırın."
else:
    try:
        genai.configure(api_key=api_key)
        # Güvenlik ayarları - İstenmeyen içerikleri engeller
        safety_settings = [{"category": "HARM_CATEGORY_HARASSMENT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"},
                           {"category": "HARM_CATEGORY_HATE_SPEECH", "threshold": "BLOCK_MEDIUM_AND_ABOVE"},
                           {"category": "HARM_CATEGORY_SEXUALLY_EXPLICIT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"},
                           {"category": "HARM_CATEGORY_DANGEROUS_CONTENT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}]
        # Güncel ve hızlı bir model seçimi
        gemini_model = genai.GenerativeModel('gemini-1.5-flash-latest', safety_settings=safety_settings)
        # İsteğe bağlı: Modeli küçük bir istekle test et
        ...
        gemini_model = None # Hata durumunda modeli None yap

# --- YARDIMCI FONKSİYONLAR ---
# (Fonksiyon tanımları, Session State başlatmadan ÖNCE olmalı)

# Metin Okuma (TTS)
tts_engine = None
try:
    tts_engine = pyttsx3.init()
    # Motoru test et (isteğe bağlı)
    # tts_engine.say("Metin okuma motoru aktif")
    # tts_engine.runAndWait()
except Exception as e:
    # Başlatılamazsa kullanıcıyı bilgilendir ama uygulamayı durdurma
    st.toast(f"⚠️ Metin okuma başlatılamadı: {e}. Sesli okuma çalışmayabilir.", icon="🔊")

def speak(text):
    """Verilen metni sesli olarak okur."""
    if not tts_engine:
        st.warning("Metin okuma motoru aktif değil.", icon="🔊")
        return
    try:
        # Motor meşgulse bekleyebilir, bu yüzden dikkatli kullanılmalı
        tts_engine.say(text)
        tts_engine.runAndWait()
    except Exception as e:
...
def scrape_url_content(url):
    ...
    try:
        ...
        for selector in selectors:
             ...
             if elements:
                  text_content=elements[0].find_all('p', limit=10) # Çok fazla paragraf almamak için limit
                  if text_content:
...
def load_chat_history():
    ...
    if os.path.exists(CHAT_HISTORY_FILE):
        try:
            ...
            st.error(f"Geçmiş dosyası ({CHAT_HISTORY_FILE}) yüklenemedi: {e}"); return []
    else: return [] # Dosya yoksa
...
def get_gemini_response(prompt, chat_history):
    ...
    try:
        ...
        # Diğer bilinen hatalar eklenebilir
...
def get_hanogt_response(user_prompt, chat_history):
    ...
    if gemini_model:
        ...
        else: response=None # Boş yanıt veya bilinmeyen durum
    # 2. Bilgi Tabanı
    if not response:
...
# Yerel Yaratıcı/Görsel Fonksiyonları (Fallback + Kural Tabanlı Görsel)
...
def generate_prompt_influenced_image(prompt):
    # Kural Tabanlı Görsel Üretici (Tam Kod)
    ...
    keyword_themes = {
        ...
        "bulut": {"bg": [(200, 200, 200), (150, 150, 150)], "shapes": [{"type": "ellipse", "color": (255, 255, 255), "pos": (random.uniform(0.1, 0.9), random.uniform(0.1, 0.4)), "size": random.uniform(0.1, 0.3)} for _ in range(3)]},
        "deniz": {"bg": [(0, 105, 148), (0, 0, 139)], "shapes": []},
        "okyanus": {"bg": [(0, 0, 139), (0, 0, 50)], "shapes": []},
        ...
        "orman": {"bg": [(50, 100, 50), (0, 50, 0)], "shapes": [{"type": "rectangle", "color": (139, 69, 19), "pos": (random.uniform(0.1, 0.9), 0.8), "size": (0.03, 0.25)} for _ in range(5)] + [{"type": "triangle", "color": (34, 139, 34), "pos": (p[1]['pos'][0], 0.65), "size": 0.15} for p in enumerate([{"pos": (random.uniform(0.1, 0.9), 0.8)} for _ in range(5)])]},
        "gece": {"bg": [(10, 10, 40), (0, 0, 0)], "shapes": [{"type": "circle", "color": (255, 255, 200, 50), "pos": (random.uniform(0, 1), random.uniform(0, 1)), "size": 0.005} for _ in range(50)]},
        "ateş": {"bg": [(150, 0, 0), (255, 100, 0)], "shapes": [{"type": "triangle", "color": (255, 165, 0), "pos": (random.uniform(0.4, 0.6), random.uniform(0.6, 0.9)), "size": random.uniform(0.05, 0.15)} for _ in range(10)]},
        "kırmızı": {"bg": [(255, 100, 100), (180, 0, 0)], "shapes": []},
        "mavi": {"bg": [(100, 100, 255), (0, 0, 180)], "shapes": []},
        "yeşil": {"bg": [(100, 255, 100), (0, 180, 0)], "shapes": []},
        "sarı": {"bg": [(255, 255, 150), (220, 180, 0)], "shapes": []},
    }
    ...
    for y in range(height): # Gradient
        ratio=y / height; r=int(bg_color1[0] * (1 - ratio) + bg_color2[0] * ratio); g=int(bg_color1[1] * (1 - ratio) + bg_color2[1] * ratio); b=int(bg_color1[2] * (1 - ratio) + bg_color2[2] * ratio); draw.line([(0, y), (width, y)], fill=(r, g, b))
    for shape in shapes_to_draw: # Tema şekilleri
    ...
    try: # Metin yazdırma
        font=ImageFont.load_default(size=24) # Daha basit font yükleme
...
# NameError almamak için fonksiyon tanımlarının yukarıda olması kritik.
...
# Eğer kullanıcı adı yoksa bu bölüm çalışır ve ana uygulama gizlenir
...
# --- ANA UYGULAMA BÖLÜMÜ ---
# Kullanıcı adı belirlendiyse bu bölüm çalışır
    ...
    with st.expander("⚙️ Ayarlar & Kişiselleştirme", expanded=False):
        # Ad Değiştirme
        # on_change ile anında güncelleme daha akıcı olabilir
        def update_name():
            st.session_state.user_name = st.session_state.change_name_input_key
            st.toast("Adınız güncellendi!") # Success yerine toast
        st.text_input(
            "Adınızı Değiştirin:",
            value=st.session_state.user_name,
            key="change_name_input_key",
            on_change=update_name # Değişiklik olduğunda fonksiyonu çağır
        )
        ...
        if uploaded_avatar is not None:
            if uploaded_avatar.size > 1 * 1024 * 1024: st.error("Dosya boyutu 1MB'den büyük olamaz!")
            else:
                st.session_state.user_avatar_bytes = uploaded_avatar.getvalue()
                st.toast("Avatar güncellendi!", icon="🖼️")
                # Avatar yükleyince sayfayı yenilemeye gerek yok, bir sonraki mesajda görünür.
                # st.rerun()
        ...
        if st.session_state.user_avatar_bytes:
            ...
            if st.button("Avatarı Kaldır", key="remove_avatar"):
                ...
                st.rerun() # Kaldırınca hemen yansıması için yenile
        else:
            st.caption("Henüz bir avatar yüklemediniz.")
        st.caption("Not: Avatar sadece bu tarayıcı oturumu için geçerlidir.")
        st.divider()

        # Geçmiş Temizleme
        if st.button("🧹 Sohbet Geçmişini Temizle", key="clear_history_main"):
            st.session_state.chat_history = []
            save_chat_history([]) # Dosyayı da temizle
            st.toast("Sohbet geçmişi temizlendi!", icon="🧹")
            time.sleep(1) # Mesajın görünmesi için kısa bekleme
            st.rerun()
    ...
    # Mod butonları için state yönetimini basitleştirelim
    new_mode = current_mode # Varsayılan olarak mevcut mod
    for i, col in enumerate(cols):
        with col:
            is_active = (modes[i] == current_mode)
            button_type = "primary" if is_active else "secondary"
            if st.button(f"{icons[i]} {modes[i]}", key=f"mode_btn_{i}", use_container_width=True, type=button_type):
                new_mode = modes[i] # Yeni modu ata

    # Eğer mod değiştiyse state'i güncelle ve rerun yap
    ...
    if app_mode == "Yazılı Sohbet":
        ...
        chat_container = st.container() # Mesajları bir konteynıra alalım
        with chat_container:
            for i, (sender, message) in enumerate(st.session_state.chat_history):
                ...
                with st.chat_message(role, avatar=display_avatar):
                    ...
                    if not is_user and "(" in sender and ")" in sender: # Kaynak belirtme
                        try: source = sender[sender.find("(")+1:sender.find(")")]
                        except: source = "AI" # Hata olursa varsayılan
                        display_name = f"({source}) "
                    st.markdown(f"{display_name}{message}") # Mesajı göster

                    # Sesli Okuma Butonu
                    if not is_user and tts_engine:
                        if st.button(f"🔊", key=f"speak_msg_{i}", help="Mesajı sesli oku"): # Buton metnini sadece ikon yap
                            speak(message)
    ...
    # -- SESLİ SOHBET (DOSYA YÜKLEME) --
        ...
        if uploaded_file is not None:
            ...
            with st.spinner("Ses dosyası işleniyor..."):
                ...
                except sr.UnknownValueError: st.error("Ses dosyasındaki konuşma anlaşılamadı."); user_prompt = None
                except sr.RequestError as e: st.error(f"Ses tanıma servisine ulaşılamadı: {e}"); user_prompt = None
    ...
    # -- YARATICI MOD --
        ...
        if creative_prompt:
            ...
            if not final_response: # Fallback
...
if st.session_state.show_main_app:
    st.markdown("---")
    st.markdown(f"<p style='text-align: center; font-size: small;'>Hanogt AI v3.3 - {st.session_state.get('user_name', 'Misafir')} için çalışıyor - 2025</p>", unsafe_allow_html=True) # Sürüm güncellendi
