import streamlit as st import requests from bs4 import BeautifulSoup import wikipedia import speech_recognition as sr import pyttsx3 import random from knowledge_base import load_knowledge, chatbot_response import os import json from PIL import Image, ImageDraw
...
def learn_from_web(query): try: wikipedia.set_lang("tr") summary = wikipedia.summary(query, sentences=2) return summary except: try: search_query = query.replace(" ", "+") url = f"https://www.google.com/search?q={search_query}" headers = {"User-Agent": "Mozilla/5.0"} response = requests.get(url, headers=headers) soup = BeautifulSoup(response.text, "html.parser") links = soup.find_all('a', href=True) for link in links: href = link['href'] if "url?q=" in href: real_link = href.split("url?q=")[1].split("&")[0] return f"Daha fazla bilgi için bu kaynağa bakabilirsin: {real_link}" return None except: return None
...
def creative_response(prompt): styles = [ "Bunu düşünnünce aklıma gelen ilk şey şudur: {}", "Belki de şböyle hayal edebiliriz: {}", "Eğer bunu bir hikaye gibi düşünürsek: {}", "Bu konuda şunu söyleyebilirim: {}", "Bence {} olabilir." ] comment = random.choice(styles).format(generate_new_idea(prompt)) return comment

def generate_new_idea(seed): topics = ["zaman yolculuğu", "mikro evrenler", "dijital rüyalar", "ışık hızında düşünce", "ses dalgalarıyla iletişim"] verbs = ["oluşturur", "dönüştürür", "yok eder", "yeniden inşa eder", "hızlandırır"] seed = seed.lower() idea = f"{seed} {random.choice(verbs)} ve {random.choice(topics)} ile birleşir." return idea.capitalize()
...
def generate_fake_image(prompt): img = Image.new('RGB', (512, 512), color=(random.randint(0,255), random.randint(0,255), random.randint(0,255))) d = ImageDraw.Draw(img) d.text((10,10), prompt, fill=(255,255,255)) return img

--- Streamlit Sayfa Ayarları ---

st.set_page_config(page_title="Hanogt AI", page_icon=":robot_face:", layout="centered")

st.sidebar.title("Hanogt AI Menü") app_mode = st.sidebar.selectbox("Mod Seçin:", ["Sohbet Botu", "Sesli Sohbet", "Yaratıcı Mod", "Görsel Üretici"])
...
if app_mode == "Sohbet Botu": st.header("Yazılı Sohbet") user_input = st.text_input("Sen:", key="chat_input")
...
elif app_mode == "Sesli Sohbet": st.header("Sesli Sohbet")

if st.button("Konusmaya Basla"):
...
elif app_mode == "Yaratıcı Mod": st.header("Yaratıcı Mod") prompt = st.text_input("Hayal gücünü serbest bırak:", key="creative_input")
...
elif app_mode == "Görsel Üretici": st.header("Görsel Üretici") image_prompt = st.text_input("Ne çizelim?", key="image_input")
...
--- Geçmiş Konuşmalara Bak ---

if chat_history and app_mode in ["Sohbet Botu", "Sesli Sohbet"]: st.subheader("Geçmiş Konuşmalara Göz At:") for sender, message in chat_history: st.write(f"{sender}: {message}")
