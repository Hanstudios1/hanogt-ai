...
if st.button("Tahmin Yap"):
    ...
    st.success(f"{input_value:.2f} için tahmin: {prediction[0]:.2f}")import streamlit as st
import numpy as np
import matplotlib.pyplot as plt
from sklearn.linear_model import LinearRegression
from sklearn.preprocessing import PolynomialFeatures
...
# --- RegressionModel sınıfı (senin önceki kodun) ---
...
class RegressionModel:
    ...
    def evaluate(self):
        ...
        metrics = {
            "Train R2": r2_score(self.X_train, y_train_pred),
            "Test R2": r2_score(self.X_test, y_test_pred),
            "Test MAE": mean_absolute_error(self.y_test, y_test_pred),
            "Test MSE": mean_squared_error(self.y_test, y_test_pred)
        }
        return metrics
...
if app_mode == "Regresyon Modeli":
    ...
    if st.button("Veri Üret ve Eğit"):
        X, y = np.random.rand(100, 1) * 10, 3 * (np.random.rand(100))**2 + 2 * (np.random.rand(100)) + np.random.randn(100) * 5
        model.train(X, y)
