...
st.info("ℹ️ Not: Basit anahtar kelime tabanlı bilgi tabanı kullanılıyor.", icon="💡")
def load_knowledge():
    # Kendi basit anahtar kelime ve yanıtlarınızı buraya ekleyin
    return {
        ...
        "hanogt kimdir": ["Ben Hanogt AI, size yardımcı olmaya çalışan bir yapay zeka.", "Hanogt AI, sorularınızı yanıtlamak için buradayım."],
...
if not api_key: gemini_init_error = "🛑 Google API Anahtarı Secrets'ta bulunamadı!"
else:
    try:
...
except Exception as e: st.toast(f"⚠️ TTS başlatılamadı: {e}.", icon="🔊")
def speak(text):
...
def log_interaction(prompt, response, source):
    ...
    try:
        ...
        # JSON Lines formatında dosyaya ekle (her satır bir JSON)
        with open(COLLECTED_DATA_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(log_entry, ensure_ascii=False) + "\n")
    except IOError as e:
        st.toast(f"⚠️ Loglama hatası: {e}", icon="📝")
    except Exception as e:
        st.toast(f"⚠️ Beklenmedik loglama hatası: {e}", icon="📝")


# Merkezi Yanıt Oluşturma Fonksiyonu
def get_hanogt_response(user_prompt, chat_history):
    """Kullanıcı istemine göre yanıt oluşturur ve loglar."""
    response=None; ai_sender="Hanogt AI"
    # 1. Gemini
    if gemini_model:
        response=get_gemini_response(user_prompt, chat_history)
        if response and not response.startswith(GEMINI_ERROR_PREFIX): ai_sender="Hanogt AI (Gemini)"; log_interaction(user_prompt, response, ai_sender); return response, ai_sender # <<< LOGLAMA EKLENDİ
    ...
    if not response:
        ...
        if kb_resp: response=kb_resp; ai_sender="Hanogt AI (Bilgi Tabanı)"; log_interaction(user_prompt, response, ai_sender); return response, ai_sender # <<< LOGLAMA EKLENDİ
    # 3. Web Arama
    if not response:
        web_resp=search_web(user_prompt);
        if web_resp: response=web_resp; ai_sender="Hanogt AI (Web)"; log_interaction(user_prompt, response, ai_sender); return response, ai_sender # <<< LOGLAMA EKLENDİ (Kaynak detayı logda var)
    # 4. Varsayılan
    if not response:
        response=random.choice(["Yanıt veremiyorum.","Anlayamadım.","Başka soru?"]); ai_sender="Hanogt AI (Varsayılan)" # Kaynağı belirtelim
        log_interaction(user_prompt, response, ai_sender) # <<< LOGLAMA EKLENDİ
...
def generate_prompt_influenced_image(prompt):
    # ... (Kural Tabanlı Görsel Kodu aynı) ...
...
# --- ANA UYGULAMA BÖLÜMÜ ---
    ...
    if app_mode == "Yazılı Sohbet":
        ...
        with chat_container:
            ...
            for i, (sender, message) in enumerate(st.session_state.chat_history):
                ...
                with st.chat_message(role, avatar=display_avatar):
                    st.markdown(message) # Mesajın kendisi

                    # AI mesajıysa butonları ekle
                    if not is_user:
                        col1, col2 = st.columns([0.1, 0.9]) # Butonlar için küçük sütunlar
                        with col1:
                            # Sesli Okuma Butonu
                            if tts_engine:
                                if st.button(f"🔊", key=f"speak_msg_{i}", help="Mesajı sesli oku"):
                                    speak(message)
                        with col2:
                            # <<< YENİ: Kopyala Özelliği (st.code ile) >>>
                            # Mesajı st.code içine koyarak Streamlit'in dahili kopyala butonunu kullan
                            st.code(message, language=None) # language=None basit metin gösterimi sağlar
        ...
        if prompt := st.chat_input(f"{st.session_state.user_name} olarak mesaj yazın..."):
            ...
            with st.spinner("🤖 Düşünüyorum..."):
                # Merkezi yanıt fonksiyonunu çağır ve logla
    ...
    # -- SESLİ SOHBET (DOSYA YÜKLEME) --
        ...
        if uploaded_file is not None:
            ...
            if user_prompt:
                ...
                # Sesli yanıta Kopyala butonu eklemek isterseniz buraya st.code(response) ekleyebilirsiniz.
    ...
    # -- YARATICI MOD --
        ...
        # ... (Kod aynı, loglama get_hanogt_response içinde yapılır) ...
         ...
         if creative_prompt:
            ...
            if gemini_model:
                 with st.spinner("✨ İlham perileri fısıldaşıyor..."):
                      ...
                      gemini_resp = get_gemini_response(gemini_creative_prompt, []) # Yaratıcı mod için geçmişi loglamayalım
                      if gemini_resp and not gemini_resp.startswith(GEMINI_ERROR_PREFIX): final_response = gemini_resp; # ai_sender zaten Hanogt AI
            ...
            # İsteğe bağlı: Yaratıcı mod yanıtlarını da loglayabilirsiniz
...
if st.session_state.show_main_app:
    st.markdown("---")
    st.markdown(f"<p style='text-align: center; font-size: small;'>Hanogt AI v3.5 - {st.session_state.get('user_name', 'Misafir')} için çalışıyor - 2025</p>", unsafe_allow_html=True) # Sürüm güncellendi
